...
export const ClientLayout = () => {
	...
	return (
		<div className='flex flex-col gap-5'>
			{/* Menú */}
			<nav className='flex justify-center gap-10 text-sm font-bold'>
				<NavLink
					to='/account/pedidos'
					className={({ isActive }) =>
						`${isActive ? 'underline' : 'hover:underline  text-cyan-800'}`
					}
				>
					Pedidos
				</NavLink>
				{
					...
					className='flex items-center gap-1 hover:text-cyan-800 hover:underline'
				...
				<button className='hover:underline  hover:text-cyan-800' onClick={handleLogout}>
					Cerrar sesión
				</button>
			</nav>
