...
export const CardProduct = ({ img, name, price, slug, variants, variant_name }: Props) => {
  ...
  const initialVariant = variant_name.length > 0 ? variants[0] : null;
  ...
  const handleAddClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
	const SetactiveVariant = variants.find(
		variant => variant.variant_name === activeVariant?.variant_name
	);
    if (activeVariant && SetactiveVariant &&activeVariant.stock > 0) {
      addItem({
        variantId: activeVariant.id,
        productId: slug,
        name,
        image: img,
        variant_name: SetactiveVariant.variant_name,
  ...
  return (
    <div className='flex flex-col gap-6 relative'>
      <Link to={`/productos/${slug}`} className='flex relative group overflow-hidden'>
        ...
        <button
          className='bg-cyan-800 border border-slate-200 absolute w-full bottom-0 py-3 rounded-3xl flex items-center justify-center gap-1 text-sm font-medium text-white hover:bg-cyan-700 translate-y-[100%] transition-all duration-300 group-hover:translate-y-0'
          ...
          {variants.map(variant => (
            <span
              ...
              className={`grid place-items-center px-3 py-1 cursor-pointer border rounded-full font-semibold ${
                activeVariant?.variant_name === variant.variant_name ? 'border-black bg-cyan-800 text-white' : ''
              }`}
